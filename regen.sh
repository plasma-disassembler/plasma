...
if [ "$1" == "force" ]; then
  ls tests/*.bin | while read file; do
     ...
     ./run_reverse.py --nosectionsname --nocolor "tests/${name}.bin" >"tests/${name}.rev"
  done

  mv tests/server.rev tests/server_main.rev
  ./run_reverse.py tests/server.bin -x=connection_handler -ns -nc >tests/server_connection_handler.rev
